      ...
      <div className="flex items-center justify-center py-12">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Generating your progress report...
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            ...
            {format(new Date(report.period.endDate), "MMM d, yyyy")}
          </p>
        </div>
          ...
          <Button
            ...
            disabled={
              !availableReports?.some(
                (r) =>
                  r.type === currentPeriod && r.offset === currentOffset + 1
              )
            }
          >
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-2xl font-bold">
                      {report.weightProgress.starting.toFixed(1)} kg
                    </p>
                    <p className="text-xs text-muted-foreground">Starting</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold">
                      {report.weightProgress.current.toFixed(1)} kg
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {report.weightProgress.percentageChange > 0 ? "+" : ""}
                      {report.weightProgress.percentageChange.toFixed(1)}%
                    </p>
                  </div>
                </div>
              </CardContent>
                  ...
                  {report.topExercises.map((exercise: any, i) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {exercise.sessions} sessions • {exercise.totalSets}{" "}
                          sets
                        </p>
                      </div>
...
function MetricCard({
  ...
  const iconColorMap = {
    ...
    purple:
      "bg-purple-100 dark:bg-purple-900/30 ring-purple-200 dark:ring-purple-800 text-purple-600 dark:text-purple-400",
    emerald:
      "bg-emerald-100 dark:bg-emerald-900/30 ring-emerald-200 dark:ring-emerald-800 text-emerald-600 dark:text-emerald-400",
    amber:
      "bg-amber-100 dark:bg-amber-900/30 ring-amber-200 dark:ring-amber-800 text-amber-600 dark:text-amber-400",
  };
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div
          className={cn(
            ...
            color === "blue" && "bg-blue-600/10 dark:bg-blue-500/10",
            color === "purple" && "bg-violet-600/10 dark:bg-violet-500/10",
            color === "emerald" && "bg-emerald-600/10 dark:bg-emerald-500/10",
            color === "amber" && "bg-amber-600/10 dark:bg-amber-500/10"
          )}
        >
          <Icon
            className={cn(
      ...
      <CardContent>
        ...
        {comparison !== undefined && previousValue !== undefined && (
          <div className="flex items-center gap-1 mt-1">
            ...
            >
              {comparison > 0 ? "+" : ""}
              {Math.round(comparison)}% vs {previousValue}
            </p>
          </div>
        )}
      </CardContent>
